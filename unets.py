...
def load_pretrained(ckpt: str, model: UNetModel, device: str, delete_keys: list[str]) -> None:
    print(f"Loading pretrained model from {ckpt}")
    d = fix_legacy_dict(th.load(ckpt, map_location=device))
    dm = model.state_dict()
    if delete_keys:
...
def save_model(
    ...
    th.save(
        ...
        os.path.join(
            save_dir,
            f"{filename_base}_ema_{ema_w}.pt",
        ),
    )